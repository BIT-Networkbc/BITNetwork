...
public class TransferAssetActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    try {
      ...
      BigInteger amount = BigIntegerUtil.newInstance(transferAssetContract.getAmount());
      ...
      Commons.adjustBalance(accountStore, ownerAccountCapsule, BigInteger.valueOf(-fee));
  ...
  public boolean validate() throws ContractValidateException {
    ...
    BigInteger assetBalance = ownerAccount.getAsset(dynamicStore, ByteArray.toStr(assetName));
    if (null == assetBalance || assetBalance.compareTo(BigInteger.ZERO) <= 0) {
      throw new ContractValidateException("assetBalance must be greater than 0.");
    }
    if (amount.compareTo(assetBalance) > 0) {
    ...
    if (toAccount != null) {
      ...
      if (assetBalance != null) {
        try {
          assetBalance = assetBalance.add(amount); //check if overflow
        } catch (Exception e) {
          logger.debug(e.getMessage(), e);
          throw new ContractValidateException(e.getMessage());
        }
