...
public class WitnessCreateActuator extends AbstractActuator {
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (accountCapsule.getBalance().compareTo(BigInteger.valueOf(dynamicStore
            .getAccountUpgradeCost())) < 0) {
  ...
  private void createWitness(final WitnessCreateContract witnessCreateContract)
    ...
    Commons
        .adjustBalance(accountStore, witnessCreateContract.getOwnerAddress().toByteArray(), BigInteger.valueOf(-cost));
    // UPD create witness fee to feeManager
//    if (dynamicStore.supportBlackHoleOptimization()) {
//      dynamicStore.burnBit(cost);
//    } else {
//      Commons.adjustBalance(accountStore, accountStore.getBlackhole(), BigInteger.valueOf(+cost));
//    }
    Commons.adjustBalance(accountStore, accountStore.getCreateWitnessFeeManagerAddress(), BigInteger.valueOf(+cost));
