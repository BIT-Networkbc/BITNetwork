...
public class ExchangeWithdrawActuator extends AbstractActuator {
  ...
  public boolean execute(Object object) throws ContractExeException {
    ...
    try {
      ...
      BigInteger tokenQuant = BigIntegerUtil.newInstance(exchangeWithdrawContract.getQuant());
...
//            .floorDiv(Math.multiplyExact(secondTokenBalance, tokenQuant), firstTokenBalance);
        ...
        exchangeCapsule.setBalance(firstTokenBalance.subtract(tokenQuant).toString(),
            secondTokenBalance.subtract(anotherTokenQuant).toString());
      } else {
...
//            .floorDiv(Math.multiplyExact(firstTokenBalance, tokenQuant), secondTokenBalance);
        ...
        exchangeCapsule.setBalance(firstTokenBalance.subtract(anotherTokenQuant).toString(),
            secondTokenBalance.subtract(tokenQuant).toString());
      }
      ...
      if (Arrays.equals(tokenID, BIT_SYMBOL_BYTES)) {
        accountCapsule.setBalance(newBalance.add(tokenQuant).toString());
      } else {
      ...
      ret.setExchangeWithdrawAnotherAmount(anotherTokenQuant.toString());
...
//          .floorDiv(Math.multiplyExact(secondTokenBalance, tokenQuant), firstTokenBalance);
      anotherTokenQuant = bigSecondTokenBalance.multiply(bigTokenQuant)
          .divideToIntegralValue(bigFirstTokenBalance).toBigIntegerExact();
      if (firstTokenBalance.compareTo(tokenQuant) < 0 || secondTokenBalance.compareTo(anotherTokenQuant) < 0) {
      ...
      BigDecimal remainder = bigSecondTokenBalance.multiply(bigTokenQuant)
          ...
          .subtract(new BigDecimal(anotherTokenQuant));
      if (remainder.divide(new BigDecimal(anotherTokenQuant)).compareTo(new BigDecimal("0.0001")) > 0) {
...
//          .floorDiv(Math.multiplyExact(firstTokenBalance, tokenQuant), secondTokenBalance);
      anotherTokenQuant = bigFirstTokenBalance.multiply(bigTokenQuant)
          .divideToIntegralValue(bigSecondTokenBalance).toBigIntegerExact();
      if (secondTokenBalance.compareTo(tokenQuant) < 0 || firstTokenBalance.compareTo(anotherTokenQuant) < 0) {
        throw new ContractValidateException("exchange balance is not enough");
      }
