...
import static org.bit.core.capsule.TransactionCapsule.getShieldTransactionHashIgnoreTypeException;
import static org.bit.core.utils.ZenChainParams.ZC_ENCCIPHERTEXT_SIZE;
import static org.bit.core.utils.ZenChainParams.ZC_OUTCIPHERTEXT_SIZE;
...
import org.bit.common.zksnark.IncrementalMerkleTreeContainer;
import org.bit.common.zksnark.JLibrustzcash;
import org.bit.common.zksnark.LibrustzcashParam.CheckOutputParams;
import org.bit.common.zksnark.LibrustzcashParam.CheckSpendParams;
import org.bit.common.zksnark.LibrustzcashParam.FinalCheckParams;
import org.bit.common.zksnark.MerkleContainer;
...
import org.bit.core.exception.ZkProofValidateException;
...
import org.bit.core.store.NullifierStore;
import org.bit.core.store.ZKProofStore;
...
import org.bit.protos.contract.ShieldContract.ReceiveDescription;
import org.bit.protos.contract.ShieldContract.ShieldedTransferContract;
import org.bit.protos.contract.ShieldContract.SpendDescription;
...
public class ShieldedTransferActuator extends AbstractActuator {
  ...
  public boolean execute(Object result)
    ...
    try {
      if (shieldedTransferContract.getTransparentFromAddress().toByteArray().length > 0) {
        executeTransparentFrom(shieldedTransferContract.getTransparentFromAddress().toByteArray(),
            BigIntegerUtil.newInstance(shieldedTransferContract.getFromAmount()), ret, fee);
      }
      Commons.adjustAssetBalanceV2(accountStore.getBlackhole(),
          CommonParameter.getInstance().getZenTokenId(), BigInteger.valueOf(fee),
    ...
    if (shieldedTransferContract.getTransparentToAddress().toByteArray().length > 0) {
      executeTransparentTo(shieldedTransferContract.getTransparentToAddress().toByteArray(),
          BigIntegerUtil.newInstance(shieldedTransferContract.getToAmount()), ret, fee);
    }

    //adjust and verify total shielded pool value
    try {
      Commons.adjustTotalShieldedPoolValue(
              BigIntegerUtil.newInstance(shieldedTransferContract.getToAmount()).subtract(BigIntegerUtil.newInstance(shieldedTransferContract.getFromAmount())).add(BigInteger.valueOf(fee)), dynamicStore);
  ...
  private void executeTransparentFrom(byte[] ownerAddress, BigInteger amount,
    ...
    try {
      Commons.adjustAssetBalanceV2(ownerAddress, CommonParameter.getInstance()
              .getZenTokenId(), amount.negate(), accountStore, assetIssueStore,
          dynamicStore);
    } catch (BalanceInsufficientException e) {
      ret.setStatus(0, code.FAILED);
      ret.setShieldedTransactionFee(fee);
  ...
  private void executeTransparentTo(byte[] toAddress, BigInteger amount, TransactionResultCapsule ret,
      long fee)
      throws ContractExeException {
    AccountStore accountStore = chainBaseManager.getAccountStore();
    AssetIssueStore assetIssueStore = chainBaseManager.getAssetIssueStore();
  ...
  private void checkProof(List<SpendDescription> spendDescriptions,
    ...
    if (CollectionUtils.isNotEmpty(spendDescriptions)
      ...
      try {
        ...
        BigInteger valueBalance;
        BigInteger totalShieldedPoolValue = dynamicStore
            .getTotalShieldedPoolValue();
        try {
          valueBalance = BigIntegerUtil.newInstance(shieldedTransferContract.getToAmount()).subtract(BigIntegerUtil.newInstance(shieldedTransferContract.getFromAmount())).add(BigInteger.valueOf(fee));
          totalShieldedPoolValue = totalShieldedPoolValue.subtract(valueBalance);
        } catch (ArithmeticException e) {
          logger.debug(e.getMessage(), e);
          throw new ZkProofValidateException(e.getMessage(), true);
        }

        if (totalShieldedPoolValue.compareTo(BigInteger.ZERO) < 0) {
  ...
  private void validateTransparent(ShieldedTransferContract shieldedTransferContract, long fee)
    ...
    AccountStore accountStore = chainBaseManager.getAccountStore();
    BigInteger fromAmount = BigIntegerUtil.newInstance(shieldedTransferContract.getFromAmount());
    BigInteger toAmount = BigIntegerUtil.newInstance(shieldedTransferContract.getToAmount());
    ...
    if (!hasTransparentFrom && fromAmount.compareTo(BigInteger.ZERO) != 0) {
    ...
    if (!hasTransparentTo && toAmount.compareTo(BigInteger.ZERO) != 0) {
    ...
    if (hasTransparentFrom) {
      ...
      BigInteger balance = getZenBalance(ownerAccount);
      if (fromAmount.compareTo(BigInteger.ZERO) <= 0) {
      ...
      if (fromAmount.compareTo(BigInteger.valueOf(fee)) <= 0) {
        throw new ContractValidateException(
    ...
    if (hasTransparentTo) {
      ...
      if (toAccount != null) {
        try {
          getZenBalance(toAccount).add(toAmount);
        } catch (ArithmeticException e) {
          logger.debug(e.getMessage(), e);
          throw new ContractValidateException(e.getMessage());
        }
  ...
  private BigInteger getZenBalance(AccountCapsule account) {
