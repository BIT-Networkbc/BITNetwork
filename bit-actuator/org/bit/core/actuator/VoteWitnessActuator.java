...
import static org.bit.core.config.Parameter.ChainConstant.MAX_VOTE_NUMBER;
  ...
  public boolean validate() throws ContractValidateException {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        sum = sum.add(BigInteger.valueOf(vote.getVoteCount()));
      }
      ...
      if (dynamicStore.supportAllowNewResourceModel()) {
        bitPower = accountCapsule.getAllBitPower();
      } else {
        bitPower = accountCapsule.getBitPower();
      }

      sum = sum.multiply(BigInteger.valueOf(BIT_PRECISION)); //bit -> drop. The vote count is based on BIT
      if (sum.compareTo(bitPower) > 0) {
        throw new ContractValidateException(
            "The total number of votes[" + sum + "] is greater than the bitPower[" + bitPower
                + "]");
      }
