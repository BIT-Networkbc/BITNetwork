package org.bit.core.net.service.relay;
...
import org.bit.common.backup.BackupManager;
import org.bit.common.backup.BackupManager.BackupStatusEnum;
import org.bit.common.crypto.SignInterface;
import org.bit.common.crypto.SignUtils;
...
import org.bit.core.net.BitNetService;
...
import org.bit.p2p.connection.Channel;
...
public class RelayService {
  ...
  public boolean checkHelloMessage(HelloMessage message, Channel channel) {
    ...
    try {
      ...
      if (flag) {
        BitNetService.getP2pConfig().getTrustNodes().add(channel.getInetAddress());
      }
      return flag;
    } catch (Exception e) {
  ...
  private void connect() {
    for (InetSocketAddress fastForwardNode : fastForwardNodes) {
      if (!BitNetService.getP2pConfig().getActiveNodes().contains(fastForwardNode)) {
        BitNetService.getP2pConfig().getActiveNodes().add(fastForwardNode);
      }
    }
  }
  ...
  private void disconnect() {
    fastForwardNodes.forEach(address -> {
      BitNetService.getP2pConfig().getActiveNodes().remove(address);
      BitNetService.getPeers().forEach(peer -> {
