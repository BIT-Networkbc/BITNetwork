package org.bit.core.net.service.effective;
...
public class EffectiveCheckService {
  ...
  public boolean isIsolateLand() {
    return (int) bitNetDelegate.getActivePeer().stream()
        .filter(PeerConnection::isNeedSyncFromUs)
        .count() == bitNetDelegate.getActivePeer().size();
  }
  ...
  private void findEffectiveNode() {
    ...
    if (cur != null) {
      bitNetDelegate.getActivePeer().forEach(p -> {
    ...
    List<Node> tableNodes = BitNetService.getP2pService().getConnectableNodes();
    tableNodes.sort(Comparator.comparingLong(node -> -node.getUpdateTime()));
    Set<InetSocketAddress> usedAddressSet = new HashSet<>();
    bitNetDelegate.getActivePeer().forEach(p -> usedAddressSet.add(p.getInetSocketAddress()));
    Optional<Node> chosenNode = tableNodes.stream()
        ...
        .filter(node -> !BitNetService.getP2pConfig().getActiveNodes()
    ...
    BitNetService.getP2pService().connect(chosenNode.get(), future -> {
