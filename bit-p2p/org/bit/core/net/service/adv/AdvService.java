package org.bit.core.net.service.adv;

import static org.bit.core.config.Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL;
import static org.bit.core.config.Parameter.NetConstants.MAX_BIT_FETCH_PER_PEER;
import static org.bit.core.config.Parameter.NetConstants.MSG_CACHE_DURATION_IN_BLOCKS;
...
import org.bit.common.utils.Time;
...
import org.bit.core.net.peer.Item;
import org.bit.core.net.peer.PeerConnection;
import org.bit.core.net.service.fetchblock.FetchBlockService;
import org.bit.core.net.service.statistics.MessageCount;
...
public class AdvService {
  ...
  private final int MAX_BIT_CACHE_SIZE = 50_000;
  ...
  private Cache<Item, Message> bitCache = CacheBuilder.newBuilder()
      .maximumSize(MAX_BIT_CACHE_SIZE).expireAfterWrite(1, TimeUnit.HOURS)
      .recordStats().build();
  ...
  @Getter
  private MessageCount bitCount = new MessageCount();
  ...
  public boolean addInv(Item item) {
    if (fastForward && item.getType().equals(InventoryType.BIT)) {
      return false;
    }

    if (item.getType().equals(InventoryType.BIT) && bitCache.getIfPresent(item) != null) {
      return false;
    }
  ...
  public Message getMessage(Item item) {
    if (item.getType() == InventoryType.BIT) {
      return bitCache.getIfPresent(item);
    } else {
  ...
  public int fastBroadcastTransaction(TransactionMessage msg) {
    ...
    List<PeerConnection> peers = bitNetDelegate.getActivePeer().stream()
    ...
    Item item = new Item(msg.getMessageId(), InventoryType.BIT);
    bitCount.add();
    bitCache.put(item, new TransactionMessage(msg.getTransactionCapsule().getInstance()));

    List<Sha256Hash> list = new ArrayList<>();
    list.add(msg.getMessageId());
    InventoryMessage inventoryMessage = new InventoryMessage(list, InventoryType.BIT);
  ...
  public void broadcast(Message msg) {
    ...
    if (msg instanceof BlockMessage) {
      ...
      blockMsg.getBlockCapsule().getTransactions().forEach(transactionCapsule -> {
        ...
        bitCache.put(new Item(tid, InventoryType.BIT),
            new TransactionMessage(transactionCapsule.getInstance()));
      });
      blockCache.put(item, msg);
    } else if (msg instanceof TransactionMessage) {
      TransactionMessage bitMsg = (TransactionMessage) msg;
      item = new Item(bitMsg.getMessageId(), InventoryType.BIT);
      bitCount.add();
      bitCache.put(item, new TransactionMessage(bitMsg.getTransactionCapsule().getInstance()));
    } else {
      logger.error("Adv item is neither block nor bit, type: {}", msg.getType());
      return;
    }
  ...
  */
  ...
  public void onDisconnect(PeerConnection peer) {
    if (!peer.getAdvInvRequest().isEmpty()) {
      peer.getAdvInvRequest().keySet().forEach(item -> {
        if (bitNetDelegate.getActivePeer().stream()
  ...
  private void consumerInvToFetch() {
    Collection<PeerConnection> peers = bitNetDelegate.getActivePeer().stream()
    ...
    synchronized (this) {
      ...
      invToFetch.forEach((item, time) -> {
        ...
        peers.stream().filter(peer -> {
          ...
          return t != null && now - t < TIMEOUT && invSender.getSize(peer) < MAX_BIT_FETCH_PER_PEER;
  ...
  class InvSender {
    ...
    public void sendInv() {
      send.forEach((peer, ids) -> ids.forEach((key, value) -> {
        if (peer.isRelayPeer() && key.equals(InventoryType.BIT)) {
          return;
        }
