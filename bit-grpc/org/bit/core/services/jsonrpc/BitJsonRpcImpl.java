...
import static org.bit.core.Wallet.CONTRACT_VALIDATE_ERROR;
import static org.bit.core.services.http.Util.setTransactionExtraData;
import static org.bit.core.services.http.Util.setTransactionPermissionId;
import static org.bit.core.services.jsonrpc.JsonRpcApiUtil.addressCompatibleToByteArray;
import static org.bit.core.services.jsonrpc.JsonRpcApiUtil.generateFilterId;
import static org.bit.core.services.jsonrpc.JsonRpcApiUtil.getEnergyUsageTotal;
import static org.bit.core.services.jsonrpc.JsonRpcApiUtil.getTransactionIndex;
import static org.bit.core.services.jsonrpc.JsonRpcApiUtil.getTxID;
import static org.bit.core.services.jsonrpc.JsonRpcApiUtil.triggerCallContract;
...
import org.bit.common.logsfilter.ContractEventParser;
import org.bit.common.logsfilter.capsule.BlockFilterCapsule;
import org.bit.common.logsfilter.capsule.LogsFilterCapsule;
...
import org.bit.common.utils.ByteUtil;
...
import org.bit.core.db2.core.Chainbase;
...
import org.bit.core.exception.HeaderNotFound;
import org.bit.core.exception.ItemNotFoundException;
import org.bit.core.exception.JsonRpcInternalException;
import org.bit.core.exception.JsonRpcInvalidParamsException;
import org.bit.core.exception.JsonRpcInvalidRequestException;
import org.bit.core.exception.JsonRpcMethodNotFoundException;
import org.bit.core.exception.JsonRpcTooManyResultException;
import org.bit.core.exception.VMIllegalException;
import org.bit.core.services.NodeInfoService;
import org.bit.core.services.http.JsonFormat;
import org.bit.core.services.http.Util;
import org.bit.core.services.jsonrpc.filters.BlockFilterAndResult;
import org.bit.core.services.jsonrpc.filters.LogBlockQuery;
import org.bit.core.services.jsonrpc.filters.LogFilter;
import org.bit.core.services.jsonrpc.filters.LogFilterAndResult;
import org.bit.core.services.jsonrpc.filters.LogFilterWrapper;
import org.bit.core.services.jsonrpc.filters.LogMatch;
import org.bit.core.services.jsonrpc.types.BlockResult;
import org.bit.core.services.jsonrpc.types.BuildArguments;
import org.bit.core.services.jsonrpc.types.CallArguments;
import org.bit.core.services.jsonrpc.types.TransactionReceipt;
import org.bit.core.services.jsonrpc.types.TransactionResult;
import org.bit.core.store.StorageRowStore;
import org.bit.core.vm.program.Storage;
import org.bit.program.Version;
...
import org.bit.protos.contract.SmartContractOuterClass.SmartContract.ABI;
...
@Slf4j(topic = "API")
@Component
public class BitJsonRpcImpl implements BitJsonRpc, Closeable {
  ...
  @Autowired
  public BitJsonRpcImpl(@Autowired NodeInfoService nodeInfoService, @Autowired Wallet wallet,
  ...
  @Override
  public String getBitBalance(String address, String blockNumOrTag)
      throws JsonRpcInvalidParamsException {
    if (EARLIEST_STR.equalsIgnoreCase(blockNumOrTag)
      ...
      if (reply != null) {
        balance = BigIntegerUtil.newInstance(reply.getBalance());
      }
  ...
  private void callTriggerConstantContract(byte[] ownerAddressByte, byte[] contractAddressByte,
      BigInteger value, byte[] data, TransactionExtention.Builder bitExtBuilder,
    ...
    TransactionCapsule bitCap = wallet.createTransactionCapsule(triggerContract,
        ContractType.TriggerSmartContract);
    Transaction bit =
        wallet.triggerConstantContract(triggerContract, bitCap, bitExtBuilder, retBuilder);
  ...
  private void estimateEnergy(byte[] ownerAddressByte, byte[] contractAddressByte,
    ...
    TransactionCapsule bitCap = wallet.createTransactionCapsule(triggerContract,
        ContractType.TriggerSmartContract);
    Transaction bit =
        wallet.estimateEnergy(triggerContract, bitCap, bitExtBuilder, retBuilder, estimateBuilder);
    bitExtBuilder.setTransaction(bit);
    bitExtBuilder.setTxid(bitCap.getTransactionId().getByteString());
    bitExtBuilder.setResult(retBuilder);
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
  ...
  private String call(byte[] ownerAddressByte, byte[] contractAddressByte, long value,
    ...
    TransactionExtention bitExt;

    try {
      callTriggerConstantContract(ownerAddressByte, contractAddressByte, BigInteger.valueOf(value), data,
          bitExtBuilder, retBuilder);
      ...
      bitExt = bitExtBuilder.build();
    }

    String result;
    if (bitExtBuilder.getTransaction().getRet(0).getRet().equals(code.SUCESS)) {
      List<ByteString> list = bitExt.getConstantResultList();
      ...
      byte[] resData = bitExtBuilder.getConstantResult(0).toByteArray();
  ...
  public String getStorageAt(String address, String storageIdx, String blockNumOrTag)
    ...
    if (EARLIEST_STR.equalsIgnoreCase(blockNumOrTag)
      ...
      storage.generateAddrHash(smartContract.getBitHash().toByteArray());
  ...
  public String estimateGas(CallArguments args) throws JsonRpcInvalidRequestException,
    ...
    if (bitExtBuilder.getTransaction().getRet(0).getRet().equals(code.FAILED)) {
      String errMsg = retBuilder.getMessage().toStringUtf8();

      byte[] data = bitExtBuilder.getConstantResult(0).toByteArray();
      ...
      if (supportEstimateEnergy) {
        ...
        return ByteArray.toJsonHex(bitExtBuilder.getEnergyUsed());
      }
  ...
  public String getCall(CallArguments transactionCall, Object blockParamObj)
    ...
    if (EARLIEST_STR.equalsIgnoreCase(blockNumOrTag)
      ...
      return call(addressData, contractAddressData, transactionCall.parseValue().longValue(),
  ...
  private TransactionJson buildCreateSmartContractTransaction(byte[] ownerAddress,
    ...
    try {
      ...
      smartBuilder
          .setAbi(abiBuilder)
          .setCallValue(args.parseValue().toString())
      ...
      rawBuilder.setFeeLimit(args.parseGas().multiply(BigInteger.valueOf(wallet.getEnergyFee())).longValue());
  ...
  private TransactionJson buildTriggerSmartContractTransaction(byte[] ownerAddress,
    ...
    try {
      ...
      Transaction bit = wallet
          .triggerContract(build.build(), new TransactionCapsule(txBuilder.build()), bitExtBuilder,
              retBuilder);
      bit = setTransactionPermissionId(args.getPermissionId(), bit);
      bitExtBuilder.setTransaction(bit);
    } catch (JsonRpcInvalidParamsException e) {
      throw new JsonRpcInvalidParamsException(e.getMessage());
    } catch (ContractValidateException e) {
    ...
    String jsonString = Util.printTransaction(bitExtBuilder.build().getTransaction(),
        args.isVisible());
    TransactionJson transactionJson = new TransactionJson();
  ...
  private TransactionJson buildTransferContractTransaction(byte[] ownerAddress,
      BuildArguments args) throws JsonRpcInvalidParamsException, JsonRpcInvalidRequestException,
      JsonRpcInternalException {
    BigInteger amount = args.parseValue();
