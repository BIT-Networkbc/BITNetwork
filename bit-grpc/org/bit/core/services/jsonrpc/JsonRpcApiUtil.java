...
import static org.bit.core.services.jsonrpc.BitJsonRpcImpl.EARLIEST_STR;
import static org.bit.core.services.jsonrpc.BitJsonRpcImpl.LATEST_STR;
import static org.bit.core.services.jsonrpc.BitJsonRpcImpl.PENDING_STR;
...
import org.bit.protos.contract.ShieldContract.ShieldedTransferContract;
...
import org.bit.protos.contract.VoteAssetContractOuterClass.VoteAssetContract;
import org.bit.protos.contract.WitnessContract.VoteWitnessContract;
import org.bit.protos.contract.WitnessContract.VoteWitnessContract.Vote;

@Slf4j(topic = "API")
public class JsonRpcApiUtil {

  public static byte[] convertToBitAddress(byte[] address) {
  ...
  public static TriggerSmartContract triggerCallContract(byte[] address, byte[] contractAddress,
      BigInteger callValue, byte[] data, BigInteger tokenValue, String tokenId) {
    TriggerSmartContract.Builder builder = TriggerSmartContract.newBuilder();

    builder.setOwnerAddress(ByteString.copyFrom(address));
    builder.setContractAddress(ByteString.copyFrom(contractAddress));
    builder.setData(ByteString.copyFrom(data));
    builder.setCallValue(callValue.toString());

    if (StringUtils.isNotEmpty(tokenId)) {
      builder.setCallTokenValue(tokenValue.toString());
  ...
  public static BigInteger getTransactionAmount(Transaction.Contract contract, String hash,
      Wallet wallet) {
  ...
  public static BigInteger getTransactionAmount(Transaction.Contract contract, String hash,
    ...
    try {
      ...
      switch (contract.getType()) {
        case TransferContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(TransferContract.class).getAmount());
          break;
        case TransferAssetContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(TransferAssetContract.class).getAmount());
          break;
        case VoteWitnessContract:
          ...
          amount = BigInteger.valueOf(voteNumber);
          break;
        case WitnessCreateContract:
          amount = BigInteger.valueOf(1000_000_000_000_000_0L);// UPD (9999_000_000L);
          break;
        case AssetIssueContract:
        case ExchangeCreateContract:
          amount = BigInteger.valueOf(1024_000_000_000_000_0L);// UPD (1024_000_000L);
          break;
        case ParticipateAssetIssueContract:
          break;
        case FreezeBalanceContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(FreezeBalanceContract.class).getFrozenBalance());
          break;
        case TriggerSmartContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(TriggerSmartContract.class).getCallValue());
          break;
        case ExchangeInjectContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(ExchangeInjectContract.class).getQuant());
          break;
        case ExchangeWithdrawContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(ExchangeWithdrawContract.class).getQuant());
          break;
        case ExchangeTransactionContract:
          amount = BigIntegerUtil.newInstance(contractParameter.unpack(ExchangeTransactionContract.class).getQuant());
          break;
        case AccountPermissionUpdateContract:
          amount = BigInteger.valueOf(100_000_000_000_000_0L);// UPD (100_000_000L);
          break;
        case ShieldedTransferContract:
          ShieldedTransferContract shieldedTransferContract = contract.getParameter()
              .unpack(ShieldedTransferContract.class);
          if (BigIntegerUtil.newInstance(shieldedTransferContract.getFromAmount()).compareTo(BigInteger.ZERO) > 0) {
            amount = BigIntegerUtil.newInstance(shieldedTransferContract.getFromAmount());
          } else if (BigIntegerUtil.newInstance(shieldedTransferContract.getToAmount()).compareTo(BigInteger.ZERO) > 0) {
            amount = BigIntegerUtil.newInstance(shieldedTransferContract.getToAmount());
          }
          break;
        case UnfreezeBalanceContract:
        case WithdrawBalanceContract:
  ...
  public static BigInteger getAmountFromTransactionInfo(String hash, ContractType contractType,
    ...
    try {
      ...
      if (transactionInfo != null) {
        ...
        switch (contractType) {
          case UnfreezeBalanceContract:
          case UnfreezeBalanceV2Contract:
            amount = BigIntegerUtil.newInstance(transactionInfo.getUnfreezeAmount());
            break;
          case WithdrawBalanceContract:
            amount = BigIntegerUtil.newInstance(transactionInfo.getWithdrawAmount());
            break;
          case ExchangeInjectContract:
            amount = BigIntegerUtil.newInstance(transactionInfo.getExchangeInjectAnotherAmount());
            break;
          case ExchangeWithdrawContract:
            amount = BigIntegerUtil.newInstance(transactionInfo.getExchangeWithdrawAnotherAmount());
            break;
          case ExchangeTransactionContract:
            amount = BigIntegerUtil.newInstance(transactionInfo.getExchangeReceivedAmount());
            break;
          case WithdrawExpireUnfreezeContract:
            amount = BigIntegerUtil.newInstance(transactionInfo.getWithdrawExpireAmount());
            break;
          case CancelAllUnfreezeV2Contract:
            amount = transactionInfo.getCancelUnfreezeV2AmountMap().values()
                .stream().map(o -> BigIntegerUtil.newInstance(o)).reduce(BigInteger.ZERO, BigInteger::add);
            break;
          default:
            break;
        }
  ...
  public static BigInteger getUnfreezeAssetAmount(byte[] addressBytes, Wallet wallet) {
    BigInteger amount = BigInteger.ZERO;
    try {
      ...
      if (assetIssueList != null) {
        if (assetIssueList.getAssetIssueCount() != 1) {
          ...
          for (FrozenSupply frozenSupply : assetIssue.getFrozenSupplyList()) {
            amount = amount.add(BigIntegerUtil.newInstance(frozenSupply.getFrozenAmount()));
          }
        }
      }
    } catch (Exception e) {
  ...
  public static BigInteger parseQuantityValue(String value) throws JsonRpcInvalidParamsException {
    BigInteger callValue = BigInteger.ZERO;

    if (StringUtils.isNotEmpty(value)) {
      try {
        callValue = BigIntegerUtil.newInstance(value);
      } catch (Exception e) {
