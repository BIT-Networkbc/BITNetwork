package org.bit.core.net.message.handshake;
...
import org.bit.protos.Discover.Endpoint;
import org.bit.protos.Protocol;
import org.bit.protos.Protocol.HelloMessage.Builder;

public class HelloMessage extends BitMessage {
  ...
  public static Endpoint getEndpointFromNode(Node node) {
    ...
    if (StringUtils.isNotEmpty(node.getHostV4())) {
      builder.setAddress(
          ByteString.copyFrom(org.bit.p2p.utils.ByteArray.fromString(node.getHostV4())));
    }
    if (StringUtils.isNotEmpty(node.getHostV6())) {
      builder.setAddressIpv6(
          ByteString.copyFrom(org.bit.p2p.utils.ByteArray.fromString(node.getHostV6())));
    }
