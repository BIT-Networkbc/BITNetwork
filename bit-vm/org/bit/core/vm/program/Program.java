package org.bit.core.vm.program;
...
import static org.bit.common.utils.ByteUtil.stripLeadingZeroes;
...
import static org.bit.protos.contract.Common.ResourceCode.UNRECOGNIZED;
...
import org.bit.common.crypto.Hash;
...
import org.bit.core.capsule.ContractStateCapsule;
...
import org.bit.core.exception.BitException;
import org.bit.core.utils.TransactionUtil;
import org.bit.core.vm.EnergyCost;
import org.bit.core.vm.MessageCall;
import org.bit.core.vm.Op;
import org.bit.core.vm.OperationRegistry;
import org.bit.core.vm.PrecompiledContracts;
...
import org.bit.core.vm.nativecontract.CancelAllUnfreezeV2Processor;
import org.bit.core.vm.nativecontract.DelegateResourceProcessor;
import org.bit.core.vm.nativecontract.FreezeBalanceProcessor;
import org.bit.core.vm.nativecontract.FreezeBalanceV2Processor;
import org.bit.core.vm.nativecontract.UnDelegateResourceProcessor;
import org.bit.core.vm.nativecontract.UnfreezeBalanceProcessor;
import org.bit.core.vm.nativecontract.UnfreezeBalanceV2Processor;
import org.bit.core.vm.nativecontract.VoteWitnessProcessor;
import org.bit.core.vm.nativecontract.WithdrawExpireUnfreezeProcessor;
import org.bit.core.vm.nativecontract.WithdrawRewardProcessor;
import org.bit.core.vm.nativecontract.param.CancelAllUnfreezeV2Param;
import org.bit.core.vm.nativecontract.param.DelegateResourceParam;
import org.bit.core.vm.nativecontract.param.FreezeBalanceParam;
import org.bit.core.vm.nativecontract.param.FreezeBalanceV2Param;
import org.bit.core.vm.nativecontract.param.UnDelegateResourceParam;
import org.bit.core.vm.nativecontract.param.UnfreezeBalanceParam;
import org.bit.core.vm.nativecontract.param.UnfreezeBalanceV2Param;
import org.bit.core.vm.nativecontract.param.VoteWitnessParam;
import org.bit.core.vm.nativecontract.param.WithdrawExpireUnfreezeParam;
import org.bit.core.vm.nativecontract.param.WithdrawRewardParam;
import org.bit.core.vm.program.invoke.ProgramInvoke;
import org.bit.core.vm.program.invoke.ProgramInvokeFactory;
import org.bit.core.vm.program.listener.CompositeProgramListener;
import org.bit.core.vm.program.listener.ProgramListenerAware;
import org.bit.core.vm.program.listener.ProgramStorageChangeListener;
import org.bit.core.vm.repository.Key;
import org.bit.core.vm.repository.Repository;
import org.bit.core.vm.trace.ProgramTrace;
import org.bit.core.vm.trace.ProgramTraceListener;
import org.bit.core.vm.utils.MUtil;
import org.bit.core.vm.utils.VoteRewardUtil;
import org.bit.protos.Protocol;
import org.bit.protos.Protocol.AccountType;
import org.bit.protos.contract.Common;
import org.bit.protos.contract.SmartContractOuterClass.SmartContract;
import org.bit.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send BIT to.
   * @param value the BIT value transferred in the internal transaction
   */
  private InternalTransaction addInternalTx(DataWord energyLimit, byte[] senderAddress,
      byte[] transferAddress,
      BigInteger value, byte[] data, String note, long nonce, Map<String, String> tokenInfo) {
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] obtainer = obtainerAddress.toBitAddress();

    if (VMConfig.allowTvmVote()) {
      withdrawRewardAndCancelVote(owner, getContractState());
    }

    BigInteger balance = getContractState().getBalance(owner);
    ...
    InternalTransaction internalTx = addInternalTx(null, owner, obtainer, balance, null,
        "suicide", nonce,
            getContractState().getAccount(owner).getAssetMapV2().entrySet().stream()
    ...
    if (FastByteComparisons.compareTo(owner, 0, 20, obtainer, 0, 20) == 0) {
      ...
      getContractState().addBalance(owner, balance.negate());
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all bit failed in suicide: %s", e.getMessage());
        }
    ...
    if (VMConfig.allowTvmFreezeV2()) {
      byte[] Inheritor =
          FastByteComparisons.isEqual(owner, obtainer)
              ...
              : obtainer;
      BigInteger expireUnfrozenBalance = transferFrozenV2BalanceToInheritor(owner, Inheritor, getContractState());
      if (expireUnfrozenBalance.compareTo(BigInteger.ZERO) > 0 && internalTx != null) {
        internalTx.setValue(internalTx.getValue().add(expireUnfrozenBalance));
      }
    }
  ...
  private void transferDelegatedResourceToInheritor(byte[] ownerAddr, byte[] inheritorAddr, Repository repo) {
    ...
    BigInteger frozenBalanceForBandwidthOfOwner = BigInteger.ZERO;
    // check if frozen for bandwidth exists
    if (ownerCapsule.getFrozenCount() != 0) {
      frozenBalanceForBandwidthOfOwner = BigIntegerUtil.newInstance(ownerCapsule.getFrozenList().get(0).getFrozenBalance());
    }
    repo.addTotalNetWeight(frozenBalanceForBandwidthOfOwner.negate().divide(BigInteger.valueOf(BIT_PRECISION)).longValue());

    BigInteger frozenBalanceForEnergyOfOwner =
        BigIntegerUtil.newInstance(ownerCapsule.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance());
    repo.addTotalEnergyWeight(frozenBalanceForEnergyOfOwner.negate().divide(BigInteger.valueOf(BIT_PRECISION)).longValue());

    // transfer all kinds of frozen balance to BlackHole
    repo.addBalance(inheritorAddr, frozenBalanceForBandwidthOfOwner.add(frozenBalanceForEnergyOfOwner));
  }

  private BigInteger transferFrozenV2BalanceToInheritor(byte[] ownerAddr, byte[] inheritorAddr, Repository repo) {
    AccountCapsule ownerCapsule = repo.getAccount(ownerAddr);
    ...
    ownerCapsule.getFrozenV2List().stream()
        .filter(freezeV2 -> BigIntegerUtil.newInstance(freezeV2.getAmount()).compareTo(BigInteger.ZERO) > 0)
        .forEach(
            freezeV2 -> {
              switch (freezeV2.getType()) {
                case BANDWIDTH:
                  inheritorCapsule.addFrozenBalanceForBandwidthV2(BigIntegerUtil.newInstance(freezeV2.getAmount()));
                  break;
                case ENERGY:
                  inheritorCapsule.addFrozenBalanceForEnergyV2(BigIntegerUtil.newInstance(freezeV2.getAmount()));
                  break;
                case BIT_POWER:
                  inheritorCapsule.addFrozenForBitPowerV2(BigIntegerUtil.newInstance(freezeV2.getAmount()));
                  break;
              }
            });
    ...
    BigInteger expireUnfrozenBalance =
        ownerCapsule.getUnfrozenV2List().stream()
            .filter(
                unFreezeV2 ->
                    BigIntegerUtil.newInstance(unFreezeV2.getUnfreezeAmount()).compareTo(BigInteger.ZERO) > 0 && unFreezeV2.getUnfreezeExpireTime() <= nowTimestamp)
            .map(x -> BigIntegerUtil.newInstance(x.getUnfreezeAmount())).reduce(BigInteger.ZERO,BigInteger::add);
    if (expireUnfrozenBalance.compareTo(BigInteger.ZERO) > 0) {
      inheritorCapsule.setBalance(inheritorCapsule.getBalance().add(expireUnfrozenBalance).toString());
      increaseNonce();
  ...
  private void withdrawRewardAndCancelVote(byte[] owner, Repository repo) {
    ...
    if (!ownerCapsule.getVotesList().isEmpty()) {
      ...
      ownerCapsule.setOldBitPower("0");
      repo.updateVotes(owner, votesCapsule);
    }
    try {
      BigInteger balance = ownerCapsule.getBalance();
      BigInteger allowance = ownerCapsule.getAllowance();
      ownerCapsule.setInstance(ownerCapsule.getInstance().toBuilder()
          .setBalance(balance.add(allowance).toString())
          .setAllowance("0")
  ...
  public boolean canSuicide() {
    ...
    boolean freezeCheck = !VMConfig.allowTvmFreeze()
        || (accountCapsule.getDelegatedFrozenBalanceForBandwidth().equals(BigInteger.ZERO)
        && accountCapsule.getDelegatedFrozenBalanceForEnergy().equals(BigInteger.ZERO));
...
//    return freezeCheck && voteCheck;
  ...
  private boolean freezeV2Check(AccountCapsule accountCapsule) {
    ...
    boolean isDelegatedResourceEmpty =
        accountCapsule.getDelegatedFrozenV2BalanceForBandwidth().equals(BigInteger.ZERO)
            && accountCapsule.getDelegatedFrozenV2BalanceForEnergy().equals(BigInteger.ZERO);
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    if (getContractState().getBalance(senderAddress).compareTo(endowment) < 0) {
      stackPushZero();
      return;
    }
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setBitHash(ByteString.copyFrom(rootTransactionId));
        }
      ...
      BigInteger oldBalance = deposit.getBalance(newAddress);
      deposit.addBalance(newAddress, oldBalance);
    }

    // [4] TRANSFER THE BALANCE
    BigInteger newBalance = BigInteger.ZERO;
    if (!byTestingSuite() && endowment.compareTo(BigInteger.ZERO) > 0) {
      try {
      ...
      deposit.addBalance(senderAddress, endowment.negate());
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = msg.getCodeAddress().toBitAddress();
    byte[] senderAddress = getContextAddress();
    ...
    BigInteger endowment;
    try {
      endowment = msg.getEndowment().value();
    } catch (ArithmeticException e) {
      if (VMConfig.allowTvmConstantinople()) {
    ...
    if (!isTokenTransfer) {
      BigInteger senderBalance = deposit.getBalance(senderAddress);
      ...
      // transfer brc10 token validation
      tokenId = String.valueOf(msg.getTokenId().longValue()).getBytes();
      BigInteger senderBalance = deposit.getTokenBalance(senderAddress, tokenId);
      if (senderBalance.compareTo(endowment) < 0) {
        stackPushZero();
        refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
        return;
    ...
    if (byTestingSuite()) {
      ...
      getResult().addCallCreate(data, contextAddress,
        ...
        && senderAddress != contextAddress && endowment.compareTo(BigInteger.ZERO) > 0) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer bit failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, endowment.negate());
        ...
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer brc10 failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addTokenBalance(senderAddress, tokenId, endowment.negate());
    ...
    if (isTokenTransfer) {
      tokenInfo.put(new String(stripLeadingZeroes(tokenId)), endowment.toString());
    }
    InternalTransaction internalTx = addInternalTx(null, senderAddress, contextAddress,
        !isTokenTransfer ? endowment : BigInteger.ZERO, data, "call", nonce,
...
//  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(address.toBitAddress());
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] bitAddr = address.toBitAddress();
    AccountCapsule account = getContractState().getAccount(bitAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(bitAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(bitAddr, contract);
        }
      } else {
  ...
  public byte[] getContextAddress() {
    return invoke.getContractAddress().toBitAddress();
  }
  ...
  public DataWord getBalance(DataWord address) {
    BigInteger balance = getContractState().getBalance(address.toBitAddress());
    return new DataWord(balance.toByteArray());
  }

  public DataWord getRewardBalance(DataWord address) {
    BigInteger rewardBalance = VoteRewardUtil.queryReward(address.toBitAddress(), getContractState());
    return new DataWord(rewardBalance.toByteArray());
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState().getContract(address.toBitAddress());
    return contract != null ? DataWord.ONE() : DataWord.ZERO();
  }

  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState().getWitness(address.toBitAddress());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    BigInteger ret = getContractState().getTokenBalance(address.toBitAddress(),
        String.valueOf(tokenId.longValue()).getBytes());
    return new DataWord(ret.toByteArray());
  }
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    ...
    if (VMConfig.allowTvmIstanbul()) {
      ...
      senderAddress = getCallerAddress().toBitAddress();
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (msg.getOpCode() == Op.CALLCODE || msg.getOpCode() == Op.DELEGATECALL) {
      ...
      contextAddress = msg.getCodeAddress().toBitAddress();
    }
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      if (msg.getOpCode() == Op.DELEGATECALL) {
        contract.setCallerAddress(getCallerAddress().toBitAddress());
      } else {
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowTvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer brc10 or BIT to non-exist address(would create one)
  ...
  public boolean freeze(DataWord receiverAddress, DataWord frozenBalance, DataWord resourceType) {
    ...
    byte[] owner = getContextAddress();
    byte[] receiver = receiverAddress.toBitAddress();

    increaseNonce();
    InternalTransaction internalTx = addInternalTx(null, owner, receiver,
        frozenBalance.value(), null,
    ...
    try {
      ...
      param.setFrozenBalance(frozenBalance.toString());
  ...
  public boolean unfreeze(DataWord receiverAddress, DataWord resourceType) {
    ...
    increaseNonce();
    InternalTransaction internalTx = addInternalTx(null, owner, receiver, BigInteger.ZERO, null,
    ...
    try {
      ...
      processor.validate(param, repository);
      BigInteger unfreezeBalance = processor.execute(param, repository);
      repository.commit();
      if (internalTx != null) {
  ...
  public long freezeExpireTime(DataWord targetAddress, DataWord resourceType) {
    byte[] owner = getContextAddress();
    byte[] target = targetAddress.toBitAddress();
    ...
    if (FastByteComparisons.isEqual(owner, target)) {
      ...
      if (resourceCode == 0) { //  for bandwidth
        if (ownerCapsule.getFrozenCount() != 0
            && !ownerCapsule.getFrozenList().get(0).getFrozenBalance().equals("0")) {
          return ownerCapsule.getFrozenList().get(0).getExpireTime();
        }
      } else if (resourceCode == 1) { // for energy
        if (!ownerCapsule.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance().equals("0")) {
      ...
      if (delegatedResourceCapsule != null) {
        if (resourceCode == 0) {
          ...
          if (!delegatedResourceCapsule.getFrozenBalanceForEnergy().equals(BigInteger.ZERO)) {
  ...
  public boolean freezeBalanceV2(DataWord frozenBalance, DataWord resourceType) {
    ...
    try {
      ...
      param.setFrozenBalance(frozenBalance.sValue());
  ...
  public boolean unfreezeBalanceV2(DataWord unfreezeBalance, DataWord resourceType) {
    ...
    try {
      ...
      param.setOwnerAddress(owner);
      param.setUnfreezeBalance(unfreezeBalance.toString());
      param.setResourceType(parseResourceCodeV2(resourceType));

      UnfreezeBalanceV2Processor processor = new UnfreezeBalanceV2Processor();
      processor.validate(param, repository);
      BigInteger unfreezeExpireBalance = processor.execute(param, repository);
      repository.commit();
      if (unfreezeExpireBalance.compareTo(BigInteger.ZERO) > 0) {
        increaseNonce();
  ...
  public BigInteger withdrawExpireUnfreeze() {
    Repository repository = getContractState().newRepositoryChild();
    byte[] owner = getContextAddress();

    increaseNonce();
    InternalTransaction internalTx = addInternalTx(null, owner, owner, BigInteger.ZERO, null,
    ...
    try {
      ...
      processor.validate(param, repository);
      BigInteger expireUnfreezeBalance = processor.execute(param, repository);
      repository.commit();
      if (internalTx != null) {
  ...
  public boolean cancelAllUnfreezeV2Action() {
    ...
    try {
      ...
      processor.validate(param, repository);
      BigInteger withdrawExpireBalance = processor.execute(param, repository);
      repository.commit();
  ...
  public boolean delegateResource(
    ...
    try {
      ...
      param.setOwnerAddress(owner);
      param.setReceiverAddress(receiver);
      param.setDelegateBalance(delegateBalance.value());
      param.setResourceType(parseResourceCodeV2(resourceType));
  ...
  public boolean unDelegateResource(
    ...
    try {
      ...
      param.setOwnerAddress(owner);
      param.setReceiverAddress(receiver);
      param.setUnDelegateBalance(unDelegateBalance.sValue());
      param.setResourceType(parseResourceCodeV2(resourceType));
  ...
  public boolean voteWitness(int witnessArrayOffset, int witnessArrayLength,
    ...
    increaseNonce();
    InternalTransaction internalTx = addInternalTx(null, owner, null, BigInteger.ZERO, null,
    ...
    try {
      ...
      for (int i = 0; i < witnessArrayLength; i++) {
        ...
        DataWord amount = new DataWord(Arrays.copyOfRange(amountArrayData,
            i * DataWord.WORD_SIZE, (i + 1) * DataWord.WORD_SIZE));
        param.addVote(witness.toBitAddress(), amount.sValue().longValueExact());
      }
      if (internalTx != null) {
  ...
  public BigInteger withdrawReward() {
    Repository repository = getContractState().newRepositoryChild();
    byte[] owner = getContextAddress();
    ...
    try {
      ...
      processor.validate(param, repository);
      BigInteger allowance = processor.execute(param, repository);
      repository.commit();
      if (internalTx != null) {
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(BitException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(BitException e) {
      return new PrecompiledContractException(e.getMessage());
    }
