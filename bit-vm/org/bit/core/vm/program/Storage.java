...
import org.bit.core.capsule.StorageRowCapsule;
...
public class Storage {
  ...
  private static byte[] addrHash(byte[] address, byte[] bitHash) {
    if (ByteUtil.isNullOrZeroArray(bitHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, bitHash));
  }

  public void generateAddrHash(byte[] bitId) {
    // update addreHash for create2
    addrHash = addrHash(address, bitId);
  }
