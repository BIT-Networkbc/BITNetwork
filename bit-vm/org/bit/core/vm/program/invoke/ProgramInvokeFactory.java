package org.bit.core.vm.program.invoke;
...
import static org.bit.common.utils.WalletUtil.generateContractAddress;
...
public class ProgramInvokeFactory {
   ...
   */
  public static ProgramInvoke createProgramInvoke(InternalTransaction.BitType bitType,
                                                  InternalTransaction.ExecutorType executorType, Transaction tx, BigInteger tokenValue, long tokenId,
                                                  Block block,
    ...
    if (bitType == BIT_CONTRACT_CREATION_TYPE) {
      ...
      BigInteger callValue = BigIntegerUtil.newInstance(contract.getNewContract().getCallValue());
      ...
      return new ProgramInvokeImpl(contractAddress, ownerAddress, ownerAddress, balance.toString(), callValue.toString(),
          tokenValue.toString(), tokenId, data, lastHash, coinbase, timestamp, number, deposit, vmStartInUs,
          vmShouldEndInUs, energyLimit);

    } else if (bitType == BIT_CONTRACT_CALL_TYPE) {
      ...
      return new ProgramInvokeImpl(address, origin, caller, balance.toString(), callValue.toString(), tokenValue.toString(), tokenId,
          data,
  ...
  public static ProgramInvoke createProgramInvoke(Program program, DataWord toAddress,
      ...
      DataWord inValue, DataWord tokenValue, DataWord tokenId, BigInteger balanceInt, byte[] dataIn,
