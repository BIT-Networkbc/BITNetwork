...
import org.bit.core.capsule.AbiCapsule;
...
import org.bit.core.capsule.CodeCapsule;
...
import org.bit.core.config.Parameter;
import org.bit.core.db.BlockIndexStore;
import org.bit.core.db.BlockStore;
import org.bit.core.db.KhaosDatabase;
import org.bit.core.db.TransactionTrace;
...
import org.bit.core.store.AbiStore;
...
import org.bit.core.store.CodeStore;
import org.bit.core.store.ContractStateStore;
...
import org.bit.core.store.DelegationStore;
...
import org.bit.core.vm.program.Program.IllegalOperationException;
...
import org.bit.protos.Protocol.DelegatedResource;
import org.bit.protos.Protocol.Votes;
import org.bit.protos.Protocol.DelegatedResourceAccountIndex;
import org.bit.protos.contract.AssetIssueContractOuterClass.AssetIssueContract;
import org.bit.protos.contract.Common;
import org.bit.protos.contract.SmartContractOuterClass.ContractState;
...
public class RepositoryImpl implements Repository {
  ...
  private static final byte[] TOTAL_BIT_POWER_WEIGHT = "TOTAL_BIT_POWER_WEIGHT".getBytes();
  ...
  public Pair<BigInteger, BigInteger> getAccountEnergyUsageBalanceAndRestoreSeconds(AccountCapsule accountCapsule) {
    long now = getHeadSlot();
    ...
    if (now >= latestConsumeTime + accountWindowSize) {
      return Pair.of(BigInteger.ZERO, BigInteger.ZERO);
    }
    ...
    long balance = (long) ((double) newEnergyUsage * totalEnergyWeight / totalEnergyLimit * BIT_PRECISION);

    return Pair.of(BigInteger.valueOf(balance), BigInteger.valueOf(restoreSlots).multiply(BigInteger.valueOf(BLOCK_PRODUCED_INTERVAL)).divide(BigInteger.valueOf(1_000)));
  }

  public Pair<BigInteger, BigInteger> getAccountNetUsageBalanceAndRestoreSeconds(AccountCapsule accountCapsule) {
    long now = getHeadSlot();
    ...
    long balance = (long) ((double) newNetUsage * totalNetWeight / totalNetLimit * BIT_PRECISION);
  ...
  private Storage getStorageInternal(byte[] address) {
    address = TransactionTrace.convertToBitAddress(address);
  ...
  public Storage getStorage(byte[] address) {
    ...
    if (contract != null) {
      ...
      if (!ByteUtil.isNullOrZeroArray(contract.getBitHash())) {
        storage.generateAddrHash(contract.getBitHash());
      }
    }
    return storage;
  }
  ...
  public BigInteger getBalance(byte[] address) {
    ...
    return accountCapsule == null ? BigInteger.ZERO : accountCapsule.getBalance();
  }
  ...
  public BigInteger addBalance(byte[] address, BigInteger value) {
    ...
    BigInteger balance = accountCapsule.getBalance();
    if (value.compareTo(BigInteger.ZERO) == 0) {
      return balance;
    }

    if (value.compareTo(BigInteger.ZERO) < 0 && balance.compareTo(value.negate()) < 0) {
      throw new RuntimeException(
          StringUtil.createReadableString(accountCapsule.createDbKey())
              + " insufficient balance");
    }
    accountCapsule.setBalance(balance.add(value).toString());
    Key key = Key.create(address);
    accountCache.put(key, Value.create(accountCapsule,
  ...
  @Override
  public BigInteger addTokenBalance(byte[] address, byte[] tokenId, BigInteger value) {
    byte[] tokenIdWithoutLeadingZero = ByteUtil.stripLeadingZeroes(tokenId);
    AccountCapsule accountCapsule = getAccount(address);
    if (accountCapsule == null) {
      accountCapsule = createAccount(address, Protocol.AccountType.Normal);
    }
    BigInteger balance = accountCapsule.getAssetV2(new String(tokenIdWithoutLeadingZero));
    if (value.compareTo(BigInteger.ZERO) == 0) {
      return balance;
    }
    ...
    if (value.compareTo(BigInteger.ZERO) >= 0) {
      ...
      accountCapsule
          .reduceAssetAmountV2(tokenIdWithoutLeadingZero, value.negate(), getDynamicPropertiesStore(),
              getAssetIssueStore());
    }
    Key key = Key.create(address);
    accountCache.put(key, Value.create(accountCapsule,
  ...
  @Override
  public BigInteger getTokenBalance(byte[] address, byte[] tokenId) {
    AccountCapsule accountCapsule = getAccount(address);
    if (accountCapsule == null) {
  ...
  public long calculateGlobalEnergyLimit(AccountCapsule accountCapsule) {
    ...
    if (frozeBalance.compareTo(BigInteger.valueOf(1_000_000_000_000_0L)) < 0) {
      return 0;
    }
    long energyWeight = frozeBalance.divide(BigInteger.valueOf(1_000_000_000_000_0L)).longValue();
    long totalEnergyLimit = getDynamicPropertiesStore().getTotalEnergyCurrentLimit();
  ...
  @Override
  public void addTotalBitPowerWeight(long amount) {
    long totalBitPowerWeight = getTotalBitPowerWeight();
    totalBitPowerWeight += amount;
    saveTotalBitPowerWeight(totalBitPowerWeight);
  }
  ...
  @Override
  public void saveTotalBitPowerWeight(long totalBitPowerWeight) {
    updateDynamicProperty(TOTAL_BIT_POWER_WEIGHT,
        new BytesCapsule(ByteArray.fromLong(totalBitPowerWeight)));
  }
  ...
  @Override
  public long getTotalBitPowerWeight() {
    return Optional.ofNullable(getDynamicProperty(TOTAL_BIT_POWER_WEIGHT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_BIT_POWER_WEIGHT"));
  }
