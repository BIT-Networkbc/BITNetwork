...
import static org.bit.protos.Protocol.Transaction.Contract.ContractType.ShieldedTransferContract;
import static org.bit.protos.Protocol.Transaction.Contract.ContractType.TransferAssetContract;
...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  public void updateUsage(AccountCapsule accountCapsule) {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().getAllowSameTokenName() == 0) {
      Map<String, BigInteger> assetMap = accountCapsule.getAssetMap();
    ...
    Map<String, BigInteger> assetMapV2 = accountCapsule.getAssetMapV2();
    Map<String, BigInteger> map = new HashMap<>(assetMapV2);
    accountCapsule.getAllFreeAssetNetUsageV2().forEach((k, v) -> {
      if (!map.containsKey(k)) {
        map.put(k, BigInteger.ZERO);
      }
    });
  ...
  @Override
  public void consume(TransactionCapsule bit, TransactionTrace trace)
      throws ContractValidateException, AccountResourceInsufficientException,
      TooBigTransactionResultException {
    List<Contract> contracts = bit.getInstance().getRawData().getContractList();
    if (bit.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().supportVM()) {
      bytesSize = bit.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = bit.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("TxId {}, bandwidth cost: {}.", bit.getTransactionId(), bytesSize);
...
//      }
  ...
  public long calculateGlobalNetLimit(AccountCapsule accountCapsule) {
    BigInteger frozeBalance = accountCapsule.getAllFrozenBalanceForBandwidth();
    if (dynamicPropertiesStore.supportUnfreezeDelay()) {
    ...
    long netWeight = frozeBalance.divide(BigInteger.valueOf(BIT_PRECISION)).longValue();
    long totalNetLimit = chainBaseManager.getDynamicPropertiesStore().getTotalNetLimit();
    BigInteger totalNetWeight = chainBaseManager.getDynamicPropertiesStore().getTotalNetWeight();
    if (dynamicPropertiesStore.allowNewReward() && totalNetWeight.compareTo(BigInteger.ZERO) <= 0) {
      return 0;
    }
    if (totalNetWeight.longValue() == 0) {
      return 0;
    }
    return (long) (netWeight * ((double) totalNetLimit / totalNetWeight.longValue()));
  }

  public long calculateGlobalNetLimitV2(BigInteger frozeBalance) {
    double netWeight = frozeBalance.divide(BigInteger.valueOf(BIT_PRECISION)).doubleValue();
    long totalNetLimit = dynamicPropertiesStore.getTotalNetLimit();
    BigInteger totalNetWeight = dynamicPropertiesStore.getTotalNetWeight();
    if (totalNetWeight.compareTo(BigInteger.ZERO) == 0) {
      return 0;
    }
    return (long) (netWeight * ((double) totalNetLimit / totalNetWeight.longValue()));
  }
