...
import org.bit.core.config.Parameter.ChainConstant;
...
import org.bit.protos.contract.Common.ResourceCode;
...
abstract class ResourceProcessor {
  ...
  abstract void consume(TransactionCapsule bit, TransactionTrace trace)
  ...
  protected boolean consumeFeeForBandwidth(AccountCapsule accountCapsule, long fee) {
    try {
      ...
      Commons.adjustBalance(accountStore, accountCapsule, BigInteger.valueOf(fee).negate());
      if (dynamicPropertiesStore.supportTransactionFeePool()) {
        ...
        dynamicPropertiesStore.burnBit(fee);
      } else {
        Commons.adjustBalance(accountStore, accountStore.getBlackhole().createDbKey(), BigInteger.valueOf(fee));
      }
