...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isBIT, long quant) {
    logger.info("IsBIT: {}.", isBIT);
    long balance = isBIT ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isBIT, long quant) {
    logger.info("IsBIT: {}.", isBIT);
    long balance = isBIT ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("Balance: {}.", balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isBIT, long supplyQuant) {
    long balance = isBIT ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isBIT) {
    long relay = exchangeToSupply(isBIT, from);
    return exchange_from_supply(!isBIT, relay);
  }
  ...
  public AccountCapsule buyStorageBytes(AccountCapsule accountCapsule, long storageBought) {
    ...
    BigInteger newBalance = accountCapsule.getBalance().subtract(BigInteger.valueOf(quant));
  ...
  public void sellStorage(AccountCapsule accountCapsule, long bytes) {
    ...
    BigInteger newBalance = accountCapsule.getBalance().add(BigInteger.valueOf(quant));
