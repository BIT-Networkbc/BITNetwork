...
import org.bit.common.utils.FileUtil;
import org.bit.common.utils.StorageUtils;
import org.bit.core.db.RevokingDatabase;
import org.bit.core.db.BitDatabase;
import org.bit.core.db2.ISession;
import org.bit.core.db2.common.DB;
import org.bit.core.db2.common.IRevokingDB;
import org.bit.core.db2.common.Key;
import org.bit.core.db2.common.Value;
import org.bit.core.db2.common.WrappedByteArray;
import org.bit.core.exception.RevokingStoreIllegalStateException;
import org.bit.core.store.CheckPointV2Store;
import org.bit.core.store.CheckTmpStore;
...
public class SnapshotManager implements RevokingDatabase {
  ...
  private void createCheckpoint() {
    BitDatabase<byte[]> checkPointStore = null;
    boolean syncFlag;
    try {
  ...
  private BitDatabase<byte[]> getCheckpointDB(String dbName) {
  ...
  private void checkV2() {
    ...
    for (String cp: cpList) {
      ...
      BitDatabase<byte[]> checkPointV2Store = getCheckpointDB(cp);
  ...
  private void recover(BitDatabase<byte[]> bitDatabase) {
    Map<String, Chainbase> dbMap = dbs.stream()
        .map(db -> Maps.immutableEntry(db.getDbName(), db))
        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    advance();
    for (Map.Entry<byte[], byte[]> e: bitDatabase.getDbSource()) {
      byte[] key = e.getKey();
